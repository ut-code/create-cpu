...
export function registerIntrinsics(store: CCStore) {
	for (const definition of Object.values(ccIntrinsicComponentDefinitions)) {
		store.components.register(definition.component);
		for (const pin of definition.inputPins) {
			store.componentPins.register(pin);
		}
		for (const pin of definition.outputPins) {
			store.componentPins.register(pin);
		}
	}
}
